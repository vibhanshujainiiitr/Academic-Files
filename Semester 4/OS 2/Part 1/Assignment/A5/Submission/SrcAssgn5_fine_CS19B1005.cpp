...
sem_t *mutex;//an array of mutexes
int v,p;
ofstream outfile;
...
void greedyColoring(list<int> m[],int*colour,bool*available,int u)//greedy algo
{
        list<int>::iterator i;
...
void append_number(int x ,list<int> m[]) {
    int start=x*v/p;
    int end=(x+1)*v/p;//partition
    for(int i=start;i<end;i++){
            int t=isexternal(i,m,start,end);
            vector<int> g1;
            if(t){ //if i is an boundary vertex
                    g1.push_back(i);
                    for (auto y : m[i]){
                         g1.push_back(y);//add all adjacent vertices to array g1
                    }
                    sort(g1.begin(), g1.end());//sort the array in order to obtain lock in increasing order of vertices to avoid deadlock
                    for(int i=0;i<g1.size();i++){
                        sem_wait(&mutex[g1[i]]);//wait on mutex
                    }
            ...
            if(t){
                for(int i=g1.size() - 1;i>=0;i--){
                    sem_post(&mutex[g1[i]]);//signal on mutex
                }
                g1.clear();//erase data in g1
             }
     }
}
int main() {
  ...
  outfile.open("Output_fine.txt");
  indata>>p>>v;
  mutex=(sem_t *)malloc(v*sizeof(sem_t));
  for(int i=0;i<v;i++){
