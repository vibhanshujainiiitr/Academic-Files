...
void new_edge( struct graph* Graph, int a, int b)
{
    ...
    struct node* newNode = new_node(b);
    newNode->next = Graph->array[a].head;
    Graph->array[b].head = newNode;

    // Needed to add the edge from b to a too, as it is an undirected graph
    newNode = new_node(a);
    newNode->next = Graph->array[b].head;
    Graph->array[b].head = newNode;
    return;
}
